...
public class Shapes_Move : MonoBehaviour
{
    ...
    void Start()
    {
        for (int i = 0; i < 5; i++)
        {
            ...
            {
                shapes[this.gameObject.transform.GetChild(i).gameObject.GetComponent<Shape_Properties>().position[0] + 1, this.gameObject.transform.GetChild(i).gameObject.GetComponent<Shape_Properties>().position[1]] = this.gameObject.transform.GetChild(i).gameObject;
            }
        }
    }
    ...
    void Update()
    {
        ...
        if (moves.Count != 0 || (nextmove[0] != 0 && nextmove[1] != 0))
        {
